<div class="bg-white p-6 rounded-lg shadow-md">
    <h3 class="text-xl font-semibold mb-4">Gestión de Operaciones</h3>
    ...
    <!-- Crear nueva Operación (solo visible en español) -->
    @if($locale === 'es')
        <div class="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <form wire:submit.prevent="store" class="flex flex-col gap-3">
                <div>
                    <label for="new-state-name" class="block text-sm font-medium text-gray-700 mb-1">Nombre de Operación:</label>
                    <input
                        id="new-state-name"
                        type="text"
                        wire:model.blur="name"
                        placeholder="Ej: Venta, Alquiler, Alquiler temporal..."
                ...
                <p class="text-xs text-gray-500 mt-1">
                    Al crear una Operación en español, se generarán automáticamente traducciones para inglés, francés y alemán.
                </p>
            </form>
        </div>
    @endif
        ...
        <table class="w-full bg-white">
            ...
            <tbody>
                @forelse($operationList as $operation)
                    <tr class="hover:bg-gray-50 {{ $editingOperationId === $operation->id ? 'bg-blue-50' : '' }}">
                        <td class="px-4 py-3 border-b">{{ $operation->id }}</td>
                        <td class="px-4 py-3 border-b">
                            {{ $operation->translations->first()?->name ?? 'Sin traducción' }}
                        </td>
                        <td class="px-4 py-3 border-b">
                            <div class="flex gap-2">
                                <button
                                    wire:click="toggleEdit({{ $operation->id }})"
                                    class="{{ $editingOperationId === $operation->id ? 'bg-gray-500' : 'bg-amber-500 hover:bg-amber-600' }} text-white px-2 py-1 rounded transition flex items-center"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-1">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                    {{ $editingOperationId === $operation->id ? 'Cancelar' : 'Editar' }}
                                </button>
                                <button
                                    wire:click="confirmDelete({{ $operation->id }})"
    ...
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
        <div class="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            ...
            <p class="mb-6">¿Estás seguro que deseas eliminar esta Operación? Esta acción no se puede deshacer.</p>
    ...
    <div
        ...
        x-on:operation-saved.window="show = true; message = $event.detail; setTimeout(() => show = false, 3000)"
        x-on:operation-updated.window="show = true; message = $event.detail; setTimeout(() => show = false, 3000)"
        x-on:operation-deleted.window="show = true; message = $event.detail; setTimeout(() => show = false, 3000)"
        x-show="show"
        x-transition
